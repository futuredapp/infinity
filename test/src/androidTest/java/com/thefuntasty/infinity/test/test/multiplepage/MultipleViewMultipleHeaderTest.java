...
import com.thefuntasty.infinity.test.adapter.MultipleViewsMultipleHeadersAdapter;
...
public class MultipleViewMultipleHeaderTest {
	...
	@Test
	public void multiplePagesMultipleViewsMultipleHeadersTest() throws InterruptedException {
		Config configMock = spy(Config.class);
		InfinityEventListener eventListener = mock(InfinityEventListener.class);
		MultipleViewsMultipleHeadersAdapter adapter = new MultipleViewsMultipleHeadersAdapter();
		...
		onView(withText("Header 0")).check(matches(isDisplayed()));
		onView(withText("Header 1")).check(matches(isDisplayed()));
		onView(ViewMatchers.withId(com.thefuntasty.infinity.test.R.id.recycler)).perform(RecyclerViewActions.scrollToPosition(10));
		onView(ViewMatchers.withId(com.thefuntasty.infinity.test.R.id.recycler)).perform(RecyclerViewActions.scrollToPosition(22));
		// necessary to make "Loading" item hide
		onView(ViewMatchers.withId(com.thefuntasty.infinity.test.R.id.recycler)).perform(RecyclerViewActions.scrollToPosition(21));
		onView(withText("Tomas 19")).check(matches(isDisplayed()));
